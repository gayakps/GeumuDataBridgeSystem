...
public class MinecraftConnection extends ChannelInboundHandlerAdapter {
  ...
  public void channelInactive(ChannelHandlerContext ctx) throws Exception {
      ...
      if (association != null && !knownDisconnect
        ...
        logger.info("{} has disconnected [ MinecraftPacket ]", association);
      }
...
//        logger.info(String.format("[Size : %,d] Packet : %s From : %s REMOTE : %s",msg.toString().length(), msg.getClass().getSimpleName(), ctx.channel().localAddress(), remoteAddress.toString()));

        MinecraftPacket pkt = (MinecraftPacket) msg;

        if ( pkt instanceof JoinGame ) {

          logger.info(String.format("[%s] JoinGame Packet 이 들어옵니다. ( From : %s )", Thread.currentThread().getName(), remoteAddress.toString()));
          JoinGame joinGamePacket = (JoinGame) msg;

          // 여기에서 세션 핸들러와 패킷을 함께 래핑합니다.
          MinecraftJoinPacketWrapper wrapper = new MinecraftJoinPacketWrapper(joinGamePacket, sessionHandler);
