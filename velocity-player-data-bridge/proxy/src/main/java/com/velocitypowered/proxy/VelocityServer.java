...
public class VelocityServer implements ProxyServer, ForwardingAudience {
  ...
  VelocityServer(final ProxyOptions options) {
    ...
    gayaSoftNetworkManager.init(this);
  ...
  void start() {
    ...
    for (RegisteredServer allServer : getAllServers()) {
      ServerInfo serverInfo = allServer.getServerInfo();
      logger.info(String.format("%s - %s:%d Registered", serverInfo.getName(), serverInfo.getAddress().getHostString(), serverInfo.getAddress().getPort()));
    }
  ...
  public void shutdown(boolean explicitExit, Component reason) {
    ...
    Runnable shutdownProcess = () -> {
      ...
      for (PlayerPersistentData value : PlayerHandler.playerPersistentDataHashMap.values()) {
        PlayerHandler.savePlayerData(value, false);
      }
