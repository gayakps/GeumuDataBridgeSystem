...
public class CompoundNBT implements INBT {
   ...
   protected static ITextComponent getNameComponent(String name) {
      if (SIMPLE_VALUE.matcher(name).matches()) {
         ...
         return (new StringTextComponent(s1)).appendSibling(itextcomponent).appendString(s1);
      }
   }
   ...
   public ITextComponent toFormattedComponent(String indentation, int indentDepth) {
      if (this.tagMap.isEmpty()) {
         ...
         for(Iterator<String> iterator = collection.iterator(); iterator.hasNext(); iformattabletextcomponent.appendSibling(iformattabletextcomponent1)) {
            ...
            iformattabletextcomponent1 = (new StringTextComponent(Strings.repeat(indentation, indentDepth + 1))).appendSibling(getNameComponent(s)).appendString(String.valueOf(':')).appendString(" ").appendSibling(this.tagMap.get(s).toFormattedComponent(indentation, indentDepth + 1));
            if (iterator.hasNext()) {
