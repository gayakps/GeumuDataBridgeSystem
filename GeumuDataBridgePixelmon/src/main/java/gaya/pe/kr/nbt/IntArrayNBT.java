...
public class IntArrayNBT extends CollectionNBT<IntNBT> {
   ...
   public ITextComponent toFormattedComponent(String indentation, int indentDepth) {
      ...
      IFormattableTextComponent iformattabletextcomponent = (new StringTextComponent("[")).appendSibling(itextcomponent).appendString(";");

      for(int i = 0; i < this.intArray.length; ++i) {
         iformattabletextcomponent.appendString(" ").appendSibling((new StringTextComponent(String.valueOf(this.intArray[i]))).mergeStyle(SYNTAX_HIGHLIGHTING_NUMBER));
